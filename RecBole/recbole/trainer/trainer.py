...
class PretrainTrainer(Trainer):
    ...
    def save_pretrained_model(self, epoch, saved_model_file):
        ...
        # save checkpoint into different components
        model_file = f"{saved_model_file}.model"
        optimizer_file = f"{saved_model_file}.optimizer"
        metadata_file = f"{saved_model_file}.metadata"
        other_params_file = f"{saved_model_file}.other_params"
        
        try:
            # Save model state dict (usually the largest part)
            self.logger.info(f"Saving model state to {model_file}")
            torch.save(self.model.state_dict(), model_file)
            
            # Save optimizer state
            self.logger.info(f"Saving optimizer state to {optimizer_file}")
            torch.save(self.optimizer.state_dict(), optimizer_file)
            
            # Save other parameters if any
            self.logger.info(f"Saving other parameters to {other_params_file}")
            torch.save(self.model.other_parameter(), other_params_file)
            
            # Save metadata (config and training info)
            metadata = {
                "config": self.config,
                "epoch": epoch,
                "cur_step": self.cur_step,
            ...
            # Save a reference file that lists all component files
            reference = {
                "model_file": model_file,
                "optimizer_file": optimizer_file,
                "metadata_file": metadata_file,
                "other_params_file": other_params_file,
            }
            torch.save(reference, saved_model_file)
            
            self.saved_model_file = saved_model_file
            self.logger.info(f"Successfully saved checkpoint components")
    ...
    def pretrain(self, train_data, verbose=True, show_progress=False):
        #newly add for restart from previous checkpoint
        # if "pre_model_path" in self.config and os.path.exists(self.config["pre_model_path"]):
        #     try:
        #         checkpoint = torch.load(self.config["pre_model_path"], map_location=self.device, weights_only=False)
        #         self.model.load_state_dict(checkpoint["state_dict"])
        #         self.optimizer.load_state_dict(checkpoint["optimizer"])
        #         self.start_epoch = checkpoint["epoch"] + 1
        #         self.logger.info(f"Resumed pretraining from epoch {self.start_epoch}")
        #     except Exception as e:
        #         path = self.config["pre_model_path"]
        #         print(f"Failed to load checkpoint at {path!r}")
        ...
        #             header = f.read(16)
        #         print(f"  first 16 raw bytes: {header!r}")
        #         raise
        # else:
        #     self.start_epoch = 0
        #     self.logger.info("No pretrain checkpoint found, starting from scratch.")
        # # end
        
        if "pre_model_path" in self.config and os.path.exists(self.config["pre_model_path"]):
            try:
                # Load the reference file that points to all components
                self.logger.info(f"Loading checkpoint reference from {self.config['pre_model_path']}")
                reference = torch.load(self.config["pre_model_path"], map_location=self.device, weights_only=False)
                
                # Load metadata
                metadata_file = reference['metadata_file']
                self.logger.info(f"Loading metadata from {metadata_file}")
                metadata = torch.load(metadata_file, map_location=self.device, weights_only=False)
                
                # Load model state dict
                model_file = reference['model_file']
                self.logger.info(f"Loading model state from {model_file}")
                model_state = torch.load(model_file, map_location=self.device, weights_only=False)
                self.model.load_state_dict(model_state)
                
                # Load optimizer state dict
                optimizer_file = reference['optimizer_file']
                self.logger.info(f"Loading optimizer state from {optimizer_file}")
                optimizer_state = torch.load(optimizer_file, map_location=self.device, weights_only=False)
                self.optimizer.load_state_dict(optimizer_state)
                
                # Load other parameters if needed
                other_params_file = reference['other_params_file']
                self.logger.info(f"Loading other parameters from {other_params_file}")
                other_params = torch.load(other_params_file, map_location=self.device, weights_only=False)
                self.model.load_other_parameter(other_params)
                ...
                path = self.config["pre_model_path"]
                self.logger.error(f"Failed to load checkpoint at {path!r}")
                self.logger.error(f"  exists: {os.path.exists(path)}, size: {os.path.getsize(path) if os.path.exists(path) else 'n/a'}")
                # Try to print some debugging info
                try:
                    # Check if we can load the reference file
                    if os.path.exists(path):
                        with open(path, "rb") as f:
                            header = f.read(min(100, os.path.getsize(path)))
                        self.logger.error(f"  first bytes: {header!r}")
                        
                        # Check if component files exist
                        try:
                            ref = torch.load(path, map_location="cpu", weights_only=False)
                            for k, v in ref.items():
                                self.logger.error(f"  {k}: exists={os.path.exists(v)}, size={os.path.getsize(v) if os.path.exists(v) else 'n/a'}")
                        except:
                            self.logger.error("  Could not load reference file")
